...
void transfrom_helpers1(sha256_info *sha, UINT (*new_data)[MAX_SIZE_OF_WORD])
{
	UCHAR* data;
	UINT i;
	UINT j;
...
void sha256_transform_words(sha256_info *sha)
{
	...
	UINT i, t1, t2, m[MAX_SIZE_OF_WORD];

	i = START_VALUE;
	transfrom_helpers1(sha, &m);
	...
	while (i < MAX_SIZE_OF_WORD)
	{
		t1 = tmp[7] + EP1(tmp[4]) + CH(tmp[4], tmp[5], tmp[6]) + sha256_word[i] + m[i];
		t2 = EP0(tmp[0]) + MAJ(tmp[0], tmp[1], tmp[2]);
		tmp[7] = tmp[6];
		tmp[6] = tmp[5];
		tmp[5] = tmp[4];
		tmp[4] = tmp[3] + t1;
...
void sha256_final(sha256_info *sha, unsigned char* hash)
{
	...
	else
	{
		...
		memset(sha->data, ZERO_BIT, MESSAGE_BYTE_SIZE);
	}
